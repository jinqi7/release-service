...
func (m Metadata) ParseStringMap(key string, stringMap map[string]string) (string, error) {
	if val, ok := stringMap[m[key]]; ok {
		...
		return "", fmt.Errorf("no valid metadta key found")
	}
}
...
func (m Metadata) MustParseString(key string) (string, error) {
	if val, ok := m[key]; ok && val != "" {
		...
		return "", fmt.Errorf("value of metadta key %s cannot be empty", key)
	}
}
...
func (m Metadata) MustNotParseString(key string, conflictValueName string) (string, error) {
	if val, ok := m[key]; ok && val != "" {
		...
		return "", fmt.Errorf("value of metadta key %s cannot exists when %s is set", key, conflictValueName)
	}
}
...
func (m Metadata) MustParseInt(key string) (int, error) {
	if val, ok := m[key]; ok && val != "" {
		...
		return 0, fmt.Errorf("metadta key %s not foud for int coneversion", val)
	}
}

func (m Metadata) MustParseInt64(key string) (int64, error) {
	if val, ok := m[key]; ok && val != "" {
		parsedVal, err := strconv.ParseInt(val, 10, 64)
		if err != nil {
			return 0, fmt.Errorf("invalid conversion error for value %s", val)
		...
		return 0, fmt.Errorf("metadta key %s not foud for int64 coneversion", val)
	}
}
...
func (m Metadata) MustParseBool(key string) (bool, error) {
	if val, ok := m[key]; ok && val != "" {
		...
		return false, fmt.Errorf("metadta key %s not foud for bool coneversion", val)
	}
}
...
func (m Metadata) MustParseStringList(key string) ([]string, error) {
	if val, ok := m[key]; ok && val != "" {
		...
		if len(list) == 0 {
			return nil, fmt.Errorf("value of metadta key %s cannot be empty", key)
		}
		return list, nil
	} else {
		return nil, fmt.Errorf("value of metadta key %s cannot be empty", key)
...
func (m Metadata) MustParseEnv(key, envVar, defaultValue string) (string, error) {
	...
	return "", fmt.Errorf("cannot extract metadta key %s from environment variable", key)
}
