...
//
// For Example: To set `Content-RequestType` and `Accept` as `application/json`.
//
//	client.R().
//		SetHeader("Content-RequestType", "application/json").
...
//			"Content-RequestType": "application/json",
...
// ExpectContentType method allows to provide fallback `Content-RequestType` for automatic unmarshalling
// when `Content-RequestType` response header is unavailable.
func (r *Request) ExpectContentType(contentType string) *Request {
	r.fallbackContentType = contentType
	return r
}

// ForceContentType method provides a strong sense of response `Content-RequestType` for automatic unmarshalling.
// Resty gives this a higher priority than the `Content-RequestType` response header.  This means that if both
// `Request.ForceContentType` is set and the response `Content-RequestType` is available, `ForceContentType` will win.
...
//	client := resty.New()
//
//	resp, err := client.R().EnableTrace().Load("https://httpbin.org/get")
