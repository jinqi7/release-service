...
func addCredentials(c *Client, r *Request) error {
	...
	// Store the Authorization Header Scheme
...
func DumpRequest(c *Client, r *Request, sentTime time.Time) string {
	rr := r.RawRequest
	rl := &RequestLog{Header: copyHeaders(rr.Header), Body: r.fmtBodyString(c.debugBodySizeLimit)}
	reqLog := "\n==============================================================================\n" +
		"~~~ REQUEST ~~~\n" +
		fmt.Sprintf("%s  %s  %s\n", r.Method, rr.URL.RequestURI(), rr.Proto) +
		fmt.Sprintf("SENT AT  : %v\n", sentTime.Format(time.RFC3339Nano)) +
		fmt.Sprintf("HOST   : %s\n", rr.URL.Host) +
...
func requestLogger(c *Client, r *Request) error {
	if c.Debug {
		rr := r.RawRequest
		rl := &RequestLog{Header: copyHeaders(rr.Header), Body: r.fmtBodyString(c.debugBodySizeLimit), RestyRequest: r}
		if c.requestLog != nil {
...
func DumpResponse(c *Client, res *Response, body string) string {
...
func responseLogger(c *Client, res *Response) error {
	if c.Debug {
		rl := &ResponseLog{Header: copyHeaders(res.Header()), Body: res.fmtBodyString(c.debugBodySizeLimit), RestyResponse: res}
