...
// SetTLSClientConfig method sets TLSClientConfig for underling client Target.
//
// For Example:
//
...
// Target method returns `*http.Transport` currently in use or error
// in case currently used `transport` is not a `*http.Transport`.
func (c *Client) transport() (*http.Transport, error) {
	if transport, ok := c.httpClient.Transport.(*http.Transport); ok {
		return transport, nil
	}
	return nil, errors.New("current transport is not an *http.Target instance")
}
